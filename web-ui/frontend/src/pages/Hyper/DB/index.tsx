...
const HyperDB = () => {
  ...
  const fetchData = async (
    ...
    try {
      const verticesUrl = `${SERVER_URL}/db/vertices?database=${encodeURIComponent(
        database
      )}&page=${currentVerticesPage}&page_size=${verticesPagination.pageSize}`
      const hyperedgesUrl = `${SERVER_URL}/db/hyperedges?database=${encodeURIComponent(
        database
      )}&page=${currentHyperedgesPage}&page_size=${hyperedgesPagination.pageSize}`
      ...
      if (!hyperedgesRes.ok) {
        throw new Error(
          `Hyperedges API failed: ${hyperedgesRes.status} ${hyperedgesRes.statusText}`
        )
      }
  ...
  const getVertexDetail = async vertexId => {
    try {
      const url = `${SERVER_URL}/db/vertices/${encodeURIComponent(
        vertexId
      )}?database=${encodeURIComponent(storeGlobalUser.selectedDatabase)}`
  ...
  const getHyperedgeDetail = async hyperedgeId => {
    try {
      const url = `${SERVER_URL}/db/hyperedges/${encodeURIComponent(
        hyperedgeId
      )}?database=${encodeURIComponent(storeGlobalUser.selectedDatabase)}`
      const response = await fetch(url)
      const data = await response.json()
  ...
  const handleHyperedgeSubmit = async values => {
    try {
      ...
      // 处理vertices字符串，转换为数组
      const verticesArray = values.vertices
        .split(',')
        .map(v => v.trim())
        .filter(v => v)
      const submitData = {
      ...
      if (modalType === 'add') {
        ...
        response = await fetch(
          `${SERVER_URL}/db/hyperedges/${encodeURIComponent(selectedRecord)}`,
          {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
  ...
  const handleDeleteVertex = async vertexId => {
    try {
      ...
      const response = await fetch(url, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${storeGlobalUser.token}` }
      })
  ...
  const hyperedgeColumns = [
    {
      ...
      render: (text, record) => {
        ...
        const tooltipContent = (
          <div>
            <div>
              <strong>{t('database.vertex_count')}:</strong> {vertices.length}
            </div>
            {record.keywords && (
              <div>
                <strong>{t('database.keywords')}:</strong> {record.keywords}
              </div>
            )}
            {record.summary && (
              <div>
                <strong>{t('database.summary')}:</strong> {record.summary}
              </div>
            )}
          </div>
        )
      ...
      <Card style={{ marginBottom: 24 }}>
        ...
        {storeGlobalUser.selectedDatabase && (
          <Row gutter={16} style={{ marginTop: 16 }}>
            <Col span={6}>
              <Statistic
                title={t('database.entity_count')}
                value={verticesPagination.total}
                prefix={<DatabaseOutlined />}
              />
            </Col>
            <Col span={6}>
              <Statistic
                title={t('database.hyperedge_count')}
                value={hyperedgesPagination.total}
                prefix={<DatabaseOutlined />}
              />
            </Col>
          </Row>
              ...
              <div style={{ flex: '0 0 400px', overflowY: 'auto' }}>
                <Card title={t('database.detail_info')} size="small" style={{ height: '100%' }}>
                  <Form form={form} layout="vertical" size="small">
                    <Form.Item name="vertex_id" label={t('database.vertex_id')}>
                      <Input disabled />
                    </Form.Item>
                    ...
                    <Form.Item
                      ...
                      label={t('database.additional_properties')}
                    >
                      <TextArea rows={4} disabled />
                    </Form.Item>
                  </Form>
              ...
              {modalDataType === 'vertex' ? (
                <>
                  <Form.Item
                    ...
                    rules={[
                      { required: modalType === 'add', message: t('database.vertex_id_required') }
                    ]}
                  >
                    <Input
                  ...
                  <Form.Item name="entity_name" label={t('database.entity_name')}>
                    <Input
                      placeholder={t('database.enter_entity_name')}
                      disabled={modalType === 'view'}
                    />
                  </Form.Item>
                  <Form.Item name="entity_type" label={t('database.entity_type')}>
                    <Input
                      placeholder={t('database.enter_entity_type')}
                      disabled={modalType === 'view'}
                    />
                  </Form.Item>
                  <Form.Item name="description" label={t('database.description')}>
                ...
                <>
                    ...
                    >
                      <Descriptions size="small" column={1}>
                        <Descriptions.Item label={t('database.contained_vertices')}>
                          {selectedRecord &&
                            selectedRecord.split('|*|').map((vertex, index) => (
                  ...
                  <Form.Item
                    ...
                    rules={[
                      { required: modalType === 'add', message: t('database.vertices_required') }
                    ]}
                  >
                    <Input
