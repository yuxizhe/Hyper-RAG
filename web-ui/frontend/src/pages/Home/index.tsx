...
const HyperRAGHome = () => {
    ...
    try {
      ...
      if (modeSettings) {
        ...
        if (
          ...
          Array.isArray(parsed.availableModes) &&
          parsed.availableModes.length > 0
        ) {
  ...
  const loadFromStorage = () => {
    try {
      const savedConversations =
        localStorage.getItem(STORAGE_KEYS.CONVERSATIONS) || JSON.stringify(defaultConversations)
              ...
              messages: conv.messages.map((msg, index) =>
                index === conv.messages.length - 1
                  ? {
                      ...
                      // 更新对比结果
                      isCompare:
                        extraData?.isCompare !== undefined ? extraData.isCompare : msg.isCompare,
  ...
  const querySingleMode = async (question, mode) => {
    if (!storeGlobalUser.token) {
      throw new Error('需要登录后才能提问')
    }
    const response = await fetch(`${SERVER_URL}/hyperrag/query`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(storeGlobalUser.token ? { Authorization: `Bearer ${storeGlobalUser.token}` } : {})
      },
      body: JSON.stringify({
        question: question,
        mode: mode,
  ...
  const handleSubmit = async () => {
    ...
    if (isCompareMode) {
      ...
      try {
        ...
        const compareResults = {
          mode1: {
            ...
            mode: compareMode1,
            response: result1.success
              ? result1.response || 'No response content'
          ...
          mode2: {
            ...
            mode: compareMode2,
            response: result2.success
              ? result2.response || 'No response content'
        ...
        updateLastMessage(
          `对比分析出错: ${error instanceof Error ? error.message : 'Unknown error'}`,
          {
            isCompare: true
          }
        )
        if (error instanceof Error && /需要登录/.test(error.message)) {
      ...
      try {
        ...
        updateLastMessage(`需要登录后才能提问。即将跳转到登录页...`)
        setTimeout(() => navigate('/login'), 500)
        if (error instanceof Error && /需要登录/.test(error.message)) {
          setTimeout(() => navigate('/login'), 500)
        }
    ...
    <div className="flex bg-gray-50">
      ...
      <div className="w-52 bg-gray-100 border-r border-gray-200 flex flex-col">
                    ...
                    {enabledModes.map(mode => {
                      ...
                      return (
                        <button
                          ...
                          className={`text-base flex items-center space-x-2 px-3 py-1.5 rounded-md font-medium transition-all duration-200 cursor-pointer w-full ${
                            compareMode1 === mode.value
              ...
              enabledModes.map(mode => {
                ...
                return (
                  <button
                    ...
                    className={`text-base flex items-center space-x-2 px-4 py-2 rounded-md font-medium transition-all duration-200 cursor-pointer ${
        ...
        <ScrollArea className="flex-1 p-2">
          <div className="space-y-2">
            {conversations.map(conv => (
              <div
                key={conv.id}
                className={`group p-1 rounded-lg cursor-pointer transition-colors ${
        ...
        <div className="p-4 border-t border-gray-200 space-y-4">
          <Button variant="outline" onClick={clearAllChats} className="w-full">
            ...
            {activeConversation?.messages.length === 0 ? (
              <div className="flex-1 flex items-center justify-center mt-20">
                <div className="text-center">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">Welcome to Hyper-RAG</h3>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="flex flex-col bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <div className="flex items-center space-x-2 mb-3">
                              <div className="flex items-center space-x-2">
                                {(() => {
                                  const mode = allModes.find(
                                    m => m.value === message.compareResults.mode1.mode
                                  )
                            ...
                            <div className="flex-1 flex flex-col">
                              ...
                              {message.compareResults.mode1.success && (
                                <div className="overflow-auto pl-2">
                                  ...
                                  {((message.compareResults.mode1.entities &&
                                    message.compareResults.mode1.entities.length > 0) ||
                                    (message.compareResults.mode1.hyperedges &&
                                      message.compareResults.mode1.hyperedges.length > 0)) && (
                                    <div className="mt-4">
                                      <RetrievalHyperGraph
                                        entities={message.compareResults.mode1.entities || []}
                                        hyperedges={message.compareResults.mode1.hyperedges || []}
                          ...
                          <div className="flex flex-col bg-green-50 border border-green-200 rounded-lg p-4">
                            <div className="flex items-center space-x-2 mb-3">
                              <div className="flex items-center space-x-2">
                                {(() => {
                                  const mode = allModes.find(
                                    m => m.value === message.compareResults.mode2.mode
                                  )
                                  const IconComponent = mode?.icon || Bot
                                  return <IconComponent className="w-4 h-4" />
                                })()}
                            ...
                            <div className="flex-1 flex flex-col">
                              ...
                              {message.compareResults.mode2.success && (
                                <div className="overflow-auto pl-2">
                                  ...
                                  {((message.compareResults.mode2.entities &&
                                    message.compareResults.mode2.entities.length > 0) ||
                                    (message.compareResults.mode2.hyperedges &&
                                      message.compareResults.mode2.hyperedges.length > 0)) && (
                                    <div className="mt-4">
                                      <RetrievalHyperGraph
                                        entities={message.compareResults.mode2.entities || []}
                                        hyperedges={message.compareResults.mode2.hyperedges || []}
            ...
            <div className="max-w-4xl mx-auto">
              <div className="flex space-x-4 items-center">
                <Textarea
                  ...
                  placeholder={
                    isCompareMode
                      ? `对比 ${getModeLabel(compareMode1)} 和 ${getModeLabel(
                          compareMode2
                        )} 的回答...`
